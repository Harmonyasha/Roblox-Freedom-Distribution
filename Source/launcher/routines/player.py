...
class obj_type(logic.bin_ssl_entry):
    ...
    def retr_version(self) -> util.versions.roblox:
        ...
        return util.versions.roblox.from_name(str(res.read(), encoding='utf-8'))
    ...
    def enable_mutex(self) -> None:
        '''
        Enables multiple instances of Roblox to run concurrently.
        '''
        if self.roblox_version.get_number() > 400:
            return
...
class arg_type(logic.bin_ssl_arg_type):
    ...
    def sanitise_user_code(self):
        if self.user_code:
            return
        res = self.send_request('/rfd/default-user-code', timeout=17)
