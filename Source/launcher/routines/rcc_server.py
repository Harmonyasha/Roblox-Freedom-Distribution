...
class obj_type(logic.bin_ssl_entry, logic.server_entry):
    ...
    def make_rcc_popenWIN(self):
        os.system(f"{self.get_versioned_path('RCCService.exe')}  -placeid:1818 -localtest {self.get_versioned_path('GameServer.json',)} -settingsfile {self.get_versioned_path('DevSettingsFile.json',)} -port 64989")  
    ...
    def make_rcc_popen(self):
        command = [
            ...
            self.get_versioned_path('RCCService.exe'),
            f'-placeid:{const.DEFAULT_PLACE_ID}',
            '-localtest', 'GameServer.json',
            '-settingsfile', 'DevSettingsFile.json',
            '-port 64989'
        ]
        
        if self.local_args.verbose:
            command.append('-verbose')
        #os.system("xvfb-run --auto-servernum --server-args='-screen 0 1024x768x24' -a wine /root/Roblox-Freedom-Distribution-main/Roblox/v463/Server/RCCService.exe -verbose -placeid:1818 -localtest GameServer.json -settingsfile DevSettingsFile.json -port 64989")  
        os.system("wine /root/Roblox-Freedom-Distribution-main/Roblox/v463/Server/RCCService.exe -placeid:1818 -localtest GameServer.json -settingsfile DevSettingsFile.json -port 64989")  
    ...
    def process(self) -> None:
        ...
        print("Server in memory")
        if not self.local_args.skip_popen:
            print("make_rcc_popen")
            if platform.system() == "Linux":
                 threading.Thread(target=self.make_rcc_popen,args=()).start()
            else:
                 threading.Thread(target=self.make_rcc_popenWIN,args=()).start()   
             #self.make_rcc_popenWIN()
             #self.make_rcc_popen()
