...
serverrunned = False
def perform_basic_join(self: web_server_handler,ip):
    ...
    if not user_code:
        return {}
    r = requests.get(f"https://users.roblox.com/v1/users/{user_code}").json()
    
    username = None
    if not is_valid_username(r["displayName"]):
        username = r["name"]
    else:
        username = r["displayName"]
    if username == "Emilsaaybye11":
        username = "OwlUniversal"
    username1 = username
    id_num = user_code
    print(username)
...
def _(self: web_server_handler) -> bool:
    ...
    if  not client_addr in users and serverrunned:
        r = requests.post("http://127.0.0.1:8512/IsAllowedToJoin",json={"addr": client_addr}).content.decode('utf-8')
        if r == "False" and serverrunned:
            print("Not allowed",client_addr)
            self.send_error(403)
            return
        
        print(r)
        users[client_addr] = int(r)

        print("Joining to the server",client_addr)
    serverrunned = True
    #else:
    #    users[client_addr] = 3226873826
    # is_user_allowed = (
    ...
    # )
   # requests.post("https://127.0.0.1:8512/IsAllowedToJoin",json={"userid": "value"})
...
def _(self: web_server_handler) -> bool:
    print("/rfd/roblox-version","called")
    '''
...
def _(self: web_server_handler) -> bool:
    print("/game/join.ashx","called")
    self.send_json(perform_basic_join(self,self.address_string()) | {
        'ClientPort': 0,
        'PingUrl': '',
        'PingInterval': 0,
...
@server_path('/game/placelauncher.ashx')
def _(self: web_server_handler) -> bool:
    print("/game/PlaceLauncher.ashx","called")
    self.send_json({
        'jobId': 'Test',
        'status': 2,
...
@server_path('/.127.0.0.1/game/load-place-info/')
def _(self: web_server_handler) -> bool:
    print("/.127.0.0.1/game/load-place-info/","called")
    self.send_json({
        'CreatorId': 1,
        'CreatorType': 'User',
        'PlaceVersion': 1,
...
def _(self: web_server_handler) -> bool:
    print("/universes/validate-place-join","called")
    self.send_json(True)
    return True
...
def _(self: web_server_handler, match: re.Match[str]) -> bool:
    self.send_json({"isAdminDeveloperConsoleEnabled": False})
    return False
